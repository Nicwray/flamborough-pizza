...
import { Phone, MapPin, Clock, ShoppingBag, Mail, Facebook, ArrowUp, Plus, Minus, X, Trash2, ArrowLeft, ChevronRight, Bike, Store, Search, CheckCircle, AlertCircle, CreditCard, User, Home, Lock, Banknote, Calendar, AlertTriangle, Volume2, VolumeX, Edit2, Loader2 } from 'lucide-react';
...
const BUSINESS_INFO = {
  ...
  slotsUrl: "https://www.flamboroughpizza.co.uk/_functions/slots",
...
const getAvailableDates = () => {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    const day = d.getDay();
    if (day === 4 || day === 5 || day === 6) { // Thursday, Friday, Saturday
      dates.push(d);
    }
  }
  return dates;
...
const WelcomeScreen = ({ onFinish }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black text-white flex flex-col items-center justify-center overflow-hidden">
      ...
      <button onClick={toggleMute} className="absolute top-4 right-4 z-30 p-2 bg-black/50 backdrop-blur-sm rounded-full text-white hover:bg-black/70 transition-colors">
        {isMuted ? <VolumeX size={24} /> : <Volume2 size={24} />}
      </button>

      <div className="relative z-20 text-center px-6 animate-fade-in-up">
        <h1 className="text-5xl md:text-7xl font-bold mb-4 font-serif tracking-tight drop-shadow-lg">{BUSINESS_INFO.name}</h1>
        <p className="text-xl md:text-2xl text-white mb-12 font-light tracking-wide drop-shadow-md">{BUSINESS_INFO.tagline}</p>
        <button onClick={onFinish} style={{ backgroundColor: '#3F3D3B' }} className="group relative inline-flex items-center justify-center px-8 py-4 text-lg font-bold text-white transition-all duration-200 rounded-full hover:opacity-90 hover:scale-105 shadow-xl">
          <span>Order Now</span>
          <ChevronRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
        </button>
      </div>
      <div className="absolute bottom-8 z-20 text-gray-200 text-sm animate-pulse font-medium drop-shadow-md">Open {BUSINESS_INFO.hours}</div>
    </div>
  );
};
  ...
  useEffect(() => {
    ...
    const checkSlots = async () => {
        ...
        try {
            ...
            console.error("Failed to check slots", e);
        } finally {
  ...
  if (!isShopOpen) {
      return (
        <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 text-center animate-in zoom-in duration-300">
            <div className="w-24 h-24 bg-red-100 rounded-full flex items-center justify-center mb-6 text-red-600"><AlertTriangle size={48} /></div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 animate-in slide-in-from-right duration-300">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
        ...
        <div className="space-y-4">
            <div>
              ...
              <div className="relative">
                <select value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="w-full p-4 pl-10 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-gray-900 outline-none appearance-none font-medium text-gray-700">
                    <option value="" disabled>Choose a date...</option>
                    {availableDates.map((date) => (
                    <option key={date.toISOString()} value={date.toISOString()}>{date.toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' })}</option>
                    ))}
                </select>
            ...
            <div>
              ...
              <div className="relative">
                {isLoadingSlots ? (
                    <div className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl flex items-center justify-center text-gray-500 text-sm">
        ...
        {errorMsg && <div className="mt-4 p-3 bg-red-50 text-red-600 rounded-lg text-sm flex items-center gap-2 animate-pulse"><AlertCircle size={16} />{errorMsg}</div>}

        <button onClick={handleStartOrder} style={{ backgroundColor: '#3F3D3B' }} className="w-full mt-8 py-4 text-white font-bold rounded-xl flex items-center justify-center gap-2 hover:opacity-90 active:scale-95 transition-all shadow-lg">
            <span>Start Ordering</span><ArrowLeft className="rotate-180" size={20} />
        </button>
      </div>
...
const CartView = ({ cart, onBack, onRemove, onUpdateQuantity, onCheckout, isShopOpen }) => {
  ...
  const checkDelivery = () => {
    ...
    setTimeout(() => {
      ...
      if (validPrefixes.some(prefix => formattedPostcode.startsWith(prefix))) setDeliveryStatus('success'); else setDeliveryStatus('error');
    }, 600);
  };
...
export default function App() {
  ...
  useEffect(() => {
    fetch(BUSINESS_INFO.menuUrl)
      ...
      .then(data => {
        ...
        if (data.status) setStoreStatus({ open: data.status.open, message: data.status.message });
      })
  ...
  if (!orderSetupComplete) return <OrderSetupView onConfirm={handleOrderSetupConfirm} isShopOpen={storeStatus.open} />;
  ...
  if (currentView === 'checkout') return <CheckoutView cart={cart} total={checkoutTotal} orderType={checkoutOrderType} onBack={() => setCurrentView('cart')} onCompleteOrder={handleCompleteOrder} initialDate={selectedSlot.date} initialTime={selectedSlot.time} />;
  ...
  return (
    <div className="min-h-screen font-sans text-gray-900" style={{ backgroundColor: '#FBF5ED' }}>
      ...
      {!storeStatus.open && (<div className="fixed bottom-0 left-0 right-0 z-50 bg-red-600 text-white p-4 text-center shadow-lg animate-in slide-in-from-bottom"><div className="flex items-center justify-center gap-2 font-bold text-lg mb-1"><AlertTriangle /><span>Shop Closed</span></div><p className="text-sm opacity-90">{storeStatus.message}</p></div>)}
      <main className="pt-48 px-4 max-w-3xl mx-auto min-h-screen pb-12">
        {categories.map((category) => ( <CategorySection key={category.id} category={category} onAddToCart={addToCart} isShopOpen={storeStatus.open} /> ))}
